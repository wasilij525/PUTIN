...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "putin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "putind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XPTN and zXPTN money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 77700, 55600));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XPTN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XPTN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, putin, (obfuscation, hypersend, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XPTN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all putin specific functionality (Masternodes, Zerocoin, HyperSend, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-XPTNstake=<n>", strprintf(_("Enable or disable staking functionality for XPTN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zXPTNstake=<n>", strprintf(_("Enable or disable staking functionality for zXPTN inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:77700"));
    ...
    strUsage += HelpMessageOpt("-backupzXPTN=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXPTN minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zXPTNbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zXPTN backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeputinamount=<n>     " + strprintf(_("Keep N XPTN anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The putin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that putin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. putin Core is shutting down."));
...
#endif
    // Make sure only a single putin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. putin Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //putin: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //putin specific: zerocoin and spork DB's
                ...
                // putin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZXPTNMinted();
                        RecalculateZXPTNSpent();
                    }
                    RecalculateXPTNSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // putin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of putin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart putin Core to complete") << "\n";
        ...
        zwalletMain = new CzXPTNWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zXPTNWallet
        uiInterface.InitMessage(_("Syncing zXPTN wallet..."));

        bool fEnableZXPTNBackups = GetBoolArg("-backupzXPTN", true);
        pwalletMain->setZXPTNAutoBackups(fEnableZXPTNBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zXPTNTracker->Init();
...
//    }
//
//    nAnonymizeputinAmount = GetArg("-anonymizeputinamount", 0);
//    if (nAnonymizeputinAmount > 999999) nAnonymizeputinAmount = 999999;
//    if (nAnonymizeputinAmount < 2) nAnonymizeputinAmount = 2;
    ...
    LogPrintf("Anonymize putin Amount %d\n", nAnonymizeputinAmount);
    ...
    /* Denominations
       ...
       For example:
       1XPTN+1000 == (.1XPTN+100)*10
       10XPTN+10000 == (1XPTN+1000)*10
    */
