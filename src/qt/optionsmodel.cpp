...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XPTN);
    ...
    if (!settings.contains("nAnonymizeputinAmount"))
        settings.setValue("nAnonymizeputinAmount", 1000);

    nAnonymizeputinAmount = settings.value("nAnonymizeputinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeputinAmount"))
        SoftSetArg("-anonymizeputinamount", settings.value("nAnonymizeputinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in putin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeputinAmount:
            return QVariant(nAnonymizeputinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeputinAmount:
            nAnonymizeputinAmount = value.toInt();
            settings.setValue("nAnonymizeputinAmount", nAnonymizeputinAmount);
            emit anonymizeputinAmountChanged(nAnonymizeputinAmount);
            break;
        case CoinControlFeatures:
