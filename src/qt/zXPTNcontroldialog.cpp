...
#include "ui_zXPTNcontroldialog.h"
...
std::set<std::string> ZXPTNControlDialog::setSelectedMints;
std::set<CMintMeta> ZXPTNControlDialog::setMints;

ZXPTNControlDialog::ZXPTNControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXPTNControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXPTNControlDialog::~ZXPTNControlDialog()
{
    delete ui;
}

void ZXPTNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXPTNControlDialog::updateList()
{
...
void ZXPTNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXPTNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXPTN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXPTNControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXPTNControlDialog::GetSelectedMints()
{
...
void ZXPTNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
