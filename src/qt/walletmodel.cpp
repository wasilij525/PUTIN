...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered putin address / amount:
...

    {
        ...
        if (recipients[0].useHyperSend && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 XPTN.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal putin:URI (putin:XyZ...?message=example)
            {
...
static void NotifyzXPTNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXPTNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXPTNReset.connect(boost::bind(NotifyzXPTNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXPTNReset.disconnect(boost::bind(NotifyzXPTNReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zXPTNTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
