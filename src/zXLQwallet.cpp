...
CzXPTNWallet::CzXPTNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zXPTN seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXPTNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXPTNSeed_deprecated()) {
                    LogPrintf("%s: Updated zXPTN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zXPTN seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zXPTN wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXPTNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXPTNCount(nCountLastUsed);
    else if (!walletdb.ReadZXPTNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXPTNWallet::Lock()
{
    seedMaster = 0;
}

void CzXPTNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXPTNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXPTN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXPTNWallet::LoadMintPoolFromDB()
{
...
void CzXPTNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXPTNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXPTNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zXPTNTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXPTNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXPTN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zXPTNTracker which also adds to database
    pwalletMain->zXPTNTracker->Add(dMint, true);
...
void CzXPTNWallet::SeedToZXPTN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXPTNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXPTNWallet::GenerateDeterministicZXPTN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXPTNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXPTNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
