...
public:
    ...
    bool WriteZXPTNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXPTNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXPTNSeed_deprecated(uint256& seed);
    bool EraseZXPTNSeed();
    bool EraseZXPTNSeed_deprecated();

    bool WriteZXPTNCount(const uint32_t& nCount);
    bool ReadZXPTNCount(uint32_t& nCount);
