...
CzXPTNTracker::CzXPTNTracker(std::string strWalletFile)
{
...
CzXPTNTracker::~CzXPTNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzXPTNTracker::Init()
{
...
bool CzXPTNTracker::Archive(CMintMeta& meta)
{
...
bool CzXPTNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzXPTNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzXPTNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzXPTNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzXPTNTracker::GetSerialHashes()
{
...
CAmount CzXPTNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzXPTNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzXPTNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzXPTNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzXPTNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzXPTNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzXPTNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzXPTNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzXPTNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzXPTNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzXPTNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXPTNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzXPTNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzXPTNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzXPTNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzXPTNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzXPTNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzXPTN from DB\n", __func__, listDeterministicDB.size());
    }
