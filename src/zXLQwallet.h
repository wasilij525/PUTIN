...
#ifndef putin_ZXPTNWALLET_H
#define putin_ZXPTNWALLET_H
...
public:
    CzXPTNWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZXPTN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZXPTN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //putin_ZXPTNWALLET_H
