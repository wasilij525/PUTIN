...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 XPTN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zXPTN send
enum ZerocoinSpendStatus {
    ZXPTN_SPEND_OKAY = 0,                            // No error
    ZXPTN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZXPTN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZXPTN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZXPTN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZXPTN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZXPTN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZXPTN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZXPTN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZXPTN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZXPTN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZXPTN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZXPTN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZXPTN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZXPTN_SPENT_USED_ZXPTN = 14,                      // Coin has already been spend
    ZXPTN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZXPTN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZXPTNBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZXPTNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzXPTNAuto) const;
    ...
    */
    ...
    CzXPTNWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzXPTNTracker> zXPTNTracker;
    ...
    void setZWallet(CzXPTNWallet* zwallet)
    {
        zwalletMain = zwallet;
        zXPTNTracker = std::unique_ptr<CzXPTNTracker>(new CzXPTNTracker(strWalletFile));
    }

    CzXPTNWallet* getZWallet() { return zwalletMain; }
    ...
    void setZXPTNAutoBackups(bool fEnabled)
    {
    ...
    /** zXPTN reset */
    boost::signals2::signal<void()> NotifyzXPTNReset;
