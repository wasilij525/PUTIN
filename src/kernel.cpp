...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZXPTNStake(spend));
    } else {
        ...
        CXPTNStake* XPTNInput = new CXPTNStake();
        XPTNInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(XPTNInput);
    }
